package run.sage.shark.common.utils;
    ...
    static {
        ...
        if (!FileUtil.exist(existFile)) {
            ...
            if (ObjectUtil.isEmpty(fileStream.getStream())) {
                throw new BaseException("RegionUtils初始化失败，原因：IP地址库数据不存在！");
            }
        ...
        try {
            ...
            throw new BaseException("RegionUtils初始化失败，原因：从ip2region.xdb文件加载内容失败！" + e.getMessage());
        }
        // 2、使用上述的 cBuff 创建一个完全基于内存的查询对象。
        try {
            SEARCHER = Searcher.newWithBuffer(cBuff);
        } catch (Exception e) {
            throw new BaseException("RegionUtils初始化失败，原因：" + e.getMessage());
        }
    }
